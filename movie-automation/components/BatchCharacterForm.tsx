...
interface BatchCharacterGptResponse {
  characters: {
    name: string;
    aliases: string[];
    description: string;
...
interface BatchCharacterFormProps {
  seriesId: string;
  onSuccess: (createdCharacters: Character[]) => void;
}

export function BatchCharacterForm({ seriesId, onSuccess }: BatchCharacterFormProps) {
  ...
  const batchCharacterPrompt = `Помоги мне создать основных персонажей для сериала "${seriesTitle || '[НАЗВАНИЕ СЕРИАЛА]'}".
  ...
  "characters": [
    {
      ...
      "aliases": ["Псевдоним 1", "Псевдоним 2", "Прозвище"],
      "description": "Подробное описание персонажа в 3-5 предложений",
      "keywords": ["ключевое_слово1", "ключевое_слово2", "..."],
      "quotes": ["Известная цитата 1", "Известная цитата 2"],
...
Сделай персонажей разнообразными, с индивидуальными чертами характера и особенностями. Придумай для каждого персонажа уникальное имя, характер и роль в сериале.
...
Возврати только валидный JSON без дополнительного текста.`;
  ...
  const getFormattedPrompt = () => {
    ...
    return `Помоги мне создать основных персонажей для сериала ${seriesPart}.
...
<span class="text-amber-500 font-semibold">ВАЖНО для поля imageUrls:</span> предоставь только прямые ссылки на изображения (заканчивающиеся на .jpg, .png или .webp), которые можно использовать в теге &lt;img src="..."&gt;.
...
Возврати только валидный JSON без дополнительного текста.`;
  ...
  const handleCopyPrompt = () => {
    navigator.clipboard.writeText(batchCharacterPrompt);
  ...
  const handleParseResponse = () => {
    try {
      ...
      const parsedData = JSON.parse(jsonText) as BatchCharacterGptResponse;
            ...
            <div className="space-y-4">            
              <div className="p-4 border rounded-md bg-muted/30">
                ...
                <p className="text-sm text-muted-foreground mb-2">
                  Скопируйте этот промпт, вставьте в ChatGPT и используйте результаты для создания основных персонажей
                </p>
